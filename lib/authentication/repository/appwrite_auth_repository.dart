...
import 'package:ink_relay/authentication/domain/ir_app_user.dart';
import 'package:rxdart/rxdart.dart';

class AppwriteAuthRepository implements AuthRepository {
  AppwriteAuthRepository(this._account, this._secureStorage) {
    log('AppwriteAuthRepository created', name: 'AppwriteAuthRepository');
    _userController = BehaviorSubject(
      onListen: () async {
        try {
          final sessionToken = await _secureStorage.read(key: _sessionKey);
          if (sessionToken != null) {
            log('Session token found', name: 'AppwriteAuthRepository');
            log('Getting user', name: 'AppwriteAuthRepository');
            final user = await _account.get();
            log('User found: ${user.email}', name: 'AppwriteAuthRepository');
            _userController.add(user.toAppUser());
          } else {
            log('No session token found', name: 'AppwriteAuthRepository');
            _userController.add(null);
          }
        } on AppwriteException catch (e, st) {
          log('Deleting session key', name: 'AppwriteAuthRepository');
          await _secureStorage.delete(key: _sessionKey);
          if (e.code == 401) {
            log('User not logged in', name: 'AppwriteAuthRepository');
            _userController.add(null);
          } else {
      ...
      onCancel: () async {
        log('Closing user controller', name: 'AppwriteAuthRepository');
  ...
  late final BehaviorSubject<IrAppUser?> _userController;
  ...
  @override
  Stream<IrAppUser?> getUser() {
    return _userController.stream;
  }

  @override
  Future<IrAppUser?> logIn(
  ...
  @override
  Future<IrAppUser?> logInWithSocial(AuthSocial socialMethod) {
  ...
  Future<IrAppUser?> signUp(
    ...
    try {
      ...
      log('Signed up user: ${user.email}', name: 'AppwriteAuthRepository');
...
extension AppwriteUserX on User {
  IrAppUser toAppUser() {
    return IrAppUser(
      id: $id,
